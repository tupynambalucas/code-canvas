...
description: Professional branching, commit, release strategy, and repository automation standards for the CodeCanvas monorepo.
sidebar_position: 2
---

# GitHub Standards

This document outlines the professional branching and release strategy for **CodeCanvas**, combining the structural discipline of **Git Flow** with the collaboration power of the **GitHub CLI (`gh`)**.
...
- **`extension`**: Changes to the Extension apps or core.
- **`studio`**: Changes to the Global studio apps or core.
...
**Example:** `feat(extension): add Pix payment reconciliation to checkout`

---

## Commit Validation & CI Hooks

To maintain code quality, styling consistency, and type safety across the **CodeCanvas** monorepo, we use an automated pipeline that checks all changes before they are committed locally and before they are merged in the cloud.
...
Running a full typecheck across all monorepo packages (`pnpm typecheck`) can take several seconds. To keep the commit process fast, we use a custom script located at [precommit-typecheck.ts](file:///D:/projects/code-canvas/tools/scripts/precommit-typecheck.ts).
...
3. It maps the changed files to their corresponding monorepo workspaces:
   - Files in `extension/` $\rightarrow$ typecheck `@codecanvas/*` packages.
   - Files in `studio/` $\rightarrow$ typecheck `@codecanvas-studio/assets`.
   - Files in `tools/` $\rightarrow$ typecheck `@code-canvas/tools`.
   - Files in `docs/` $\rightarrow$ typecheck `@code-canvas/docs`.
   - Files in `studio/` $\rightarrow$ typecheck `@codecanvas/*`.
4. If global configuration or root files (e.g. `package.json`, `eslint.config.ts`, `pnpm-workspace.yaml`) are modified, the script runs a full typecheck (excluding `studio` until it is updated).
5. It executes the targeted typecheck in parallel using Turborepo filters (e.g., `npx turbo run typecheck --filter=@codecanvas/*`).
...
The rules are declared in the root [package.json](file:///D:/projects/code-canvas/package.json):
...
The workflow configuration is defined in [.github/workflows/ci.yaml](file:///D:/projects/code-canvas/.github/workflows/ci.yaml):

- It installs dependencies using `pnpm install --frozen-lockfile`.
- It executes the full typecheck suite: `pnpm typecheck --filter=!@codecanvas/*`.
- It validates code style and quality: `pnpm lint --filter=!@codecanvas/*`.
