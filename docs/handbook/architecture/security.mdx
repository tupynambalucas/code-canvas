---
title: Security
description: Comprehensive overview of security measures, bot protection, and brute-force mitigation in CodeCanvas.
sidebar_position: 4
---

# Security Architecture

CodeCanvas implements a multi-layered security strategy to protect community data and ensure the integrity of sharing cycles. Our approach combines industry-standard protocols with specialized bot protection and brute-force mitigation.
...
- **Isolation**: Each form (Login/Register) maintains its own Turnstile extension via unique React keys to ensure fresh validation on mode toggle.
...
- **Mandatory DTOs**: The `LoginDTOSchema` and `RegisterDTOSchema` in `@codecanvas/extension` strictly require the `turnstileToken`.
...
The following security keys are mandatory for the **Extension** stack to operate. Each variable lives in the env file owned by the responsible application, following the project's **Bounded Context** isolation principle.
