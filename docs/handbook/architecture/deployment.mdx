---
title: Deployment
description: Production and staging deployment workflows, build-time compilation, environment setups, and self-hosted excellence in CodeCanvas.
sidebar_position: 6
---
...
The architecture is built on the **"Singleton Platform / Multi-Tenant Extension"** model. Each deployment environment consists of a specific runtime boundary:
...
Check the [Command Reference](../commands-reference.mdx#extension) for more start/stop scenarios.

---

## 4. Reverse Proxying & SSL Termination

CodeCanvas utilizes a reverse proxy setup (either **Nginx** or **Traefik**) to handle TLS termination, compression, and request routing.
...
Docker Compose automatically updates containers with minimal downtime when executing `pnpm extension:prod` with existing containers. To completely eliminate request dropouts, implement a rolling deployment pattern utilizing an upstream load balancer.
:::
