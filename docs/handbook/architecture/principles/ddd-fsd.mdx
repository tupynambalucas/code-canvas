...
description: Leveraging Domain-Driven Design (DDD) Bounded Contexts and Feature-Sliced Design (FSD) in CodeCanvas frontends.
---
...
In CodeCanvas, the combination of DDD and FSD governs both the folder structure and the import rules:

### A. Bounded Context Segregation

The monorepo is split into distinct contexts (`extension/` and `studio/`) at the root level. These boundaries ensure that community operations (Extension) and global SaaS onboarding workflows (studio) remain structurally and logically independent. Shared design tokens or brand assets are imported exclusively from the `@codecanvas-studio/assets` workspace.
