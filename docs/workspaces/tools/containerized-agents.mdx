...
The agent containers run inside the same `codecanvas-mcp-net` bridge network as the MCP gateway, communicating with it via internal DNS routing. No host-level port exposure is required for inter-service communication.
