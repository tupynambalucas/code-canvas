---
title: Docs Workspace (CodeCanvas Docs)
sidebar_label: Overview
sidebar_position: 1
slug: /docs
---

The Docs workspace manages the official documentation hub for the CodeCanvas project, known as **CodeCanvas Docs**. Built on Docusaurus, it serves as the central developer studio, housing architectural guidelines, style guides, product planning documents, and automated project changelogs and roadmaps.
...
CodeCanvas Docs is configured with a multi-extension documentation setup using Docusaurus plugins to segment the different areas of the developer studio with independent left sidebars:
...
CodeCanvas Docs fully supports English (`en`) and Portuguese (`pt-BR`) locales. Localized files mirror the structure of English documents:
...
To maintain clean repository standards, CodeCanvas Docs integrates with build-time script compiles to write unified markdown documentation at the repository root level on deployment:
...
- **CI/CD Integration**: Both compiler scripts run inside the `.github/workflows/deploy-docs.yaml` workflow to ensure root-level files stay 100% in sync with the developer studio.

## Workspace Scripts

Run these commands from the root directory to manage the CodeCanvas Docs context:
