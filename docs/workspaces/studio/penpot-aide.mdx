...
CodeCanvas utilizes a distributed Penpot stack integrated with Aide, an AI-native engineering assistant. This configuration ensures absolute data sovereignty and automated synchronization between design layers and the codebase.
...
- **Component Reuse:** Always check the existing component library (Penpot) or the UI library (`extension/apps/web/src/components`) before creating new elements. Aide must prioritize existing design system tokens and components.
...
- **SVG Optimization:** Ensure exported SVG code is optimized, follows the project's icon standards, and is integrated via the `@codecanvas-studio/assets/icons` registry.
