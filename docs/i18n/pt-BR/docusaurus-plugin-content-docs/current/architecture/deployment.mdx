...
description: Fluxos de trabalho de implantação em produção e homologação/staging, compilação em tempo de build, configuração de ambientes e excelência auto-hospedada no CodeCanvas.
...
Para garantir alta disponibilidade, segurança e ambientes reproduzíveis, o **CodeCanvas** utiliza um modelo de implantação moderno e auto-hospedado (self-hosted) projetado para máquinas virtuais na **Hetzner Cloud**. Este guia descreve nossa estratégia de implantação, sincronização de ambientes e procedimentos operacionais.

---

## 1. Topologia e Promoção de Ambientes

A arquitetura é construída com base no modelo **"Singleton Platform / Multi-Tenant Extension"**. Cada ambiente de implantação consiste em um limite de execução específico:
...
graph TD
    ...
    subgraph Staging_Env
        ...
        StgAPI["@codecanvas/extension (Staging)"]
        StgWeb["@codecanvas/extension (Staging)"]
        StgDB[("MongoDB Atlas (Staging)")]
    end
    subgraph Prod_Env
        ProdCompose["docker compose -f compose.prod.yaml --env-file .env.prod"]
        ProdAPI["@codecanvas/extension (Prod)"]
        ProdWeb["@codecanvas/extension (Prod)"]
...
Consulte a [Referência de Comandos](../commands-reference.mdx#extension) para a lista completa de comandos de implantação e passos sequenciais de lançamento.

---

## 4. Proxy Reverso e Terminação SSL

O CodeCanvas utiliza uma configuração de proxy reverso (como **Nginx** ou **Traefik**) para lidar com terminação TLS, compressão e roteamento de requisições.
...
O Docker Compose atualiza automaticamente os containers com impacto mínimo de indisponibilidade ao executar `pnpm extension:prod` com containers já existentes. Para eliminar completamente quedas de requisições, configure um padrão de implantação rotativo utilizando um balanceador de carga a montante.
:::
