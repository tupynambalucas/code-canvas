...
description: Visão geral abrangente das medidas de segurança, proteção contra bots e mitigação de brute-force no CodeCanvas.
---

# Arquitetura de Segurança

O CodeCanvas implementa uma estratégia de segurança em múltiplas camadas para proteger os dados da comunidade e garantir a integridade dos ciclos de compartilhamento. Nossa abordagem combina protocolos padrão da indústria com proteção especializada contra bots e mitigação de ataques de força bruta (brute-force).
...
- **Isolamento**: Cada formulário (Login/Registro) mantém seu próprio widget do Turnstile via chaves React exclusivas para garantir uma validação nova ao alternar o modo.
...
- **DTOs Obrigatórios**: Os esquemas `LoginDTOSchema` e `RegisterDTOSchema` no `@codecanvas/extension` exigem estritamente o `turnstileToken`.
...
As seguintes chaves de segurança são obrigatórias para a operação da stack **Extension**. Cada variável reside no arquivo env pertencente à aplicação responsável, seguindo o princípio de isolamento de **Contexto Delimitado** do projeto.

### 4.1. API (`extension/apps/api/.env.*`)
...
### 4.2. Web (`extension/apps/web/.env.*` / Docker `ARG`)
