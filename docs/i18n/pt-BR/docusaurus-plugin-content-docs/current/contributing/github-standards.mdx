...
description: Estratégia profissional de ramificação (branching), commits e releases para o monorepo CodeCanvas.
sidebar_position: 2
---

# Padrões do GitHub

Este documento descreve a estratégia profissional de ramificação e lançamento para o **CodeCanvas**, combinando a disciplina estrutural do **Git Flow** com o poder de colaboração da **GitHub CLI (`gh`)**.
...
- **`extension`**: Mudanças nos apps ou core da VS Code Extension.
- **`studio`**: Mudanças nos apps ou core do studio Global.
...
**Exemplo:** `feat(extension): add Pix payment reconciliation to checkout`

---

## Validação de Commits & Hooks de CI

Para manter a qualidade do código, a consistência de estilo e a segurança de tipos em todo o monorepo **CodeCanvas**, utilizamos um fluxo automatizado que valida todas as alterações antes que elas sejam salvas localmente e antes de sofrerem merge na nuvem.
...
Rodar a checagem completa de tipos em todas as aplicações (`pnpm typecheck`) a cada commit pode ser lento. Para otimizar esse fluxo, desenvolvemos um script inteligente em [precommit-typecheck.ts](file:///D:/projects/code-canvas/tools/scripts/precommit-typecheck.ts).
...
3. Mapeia os arquivos modificados para seus respectivos pacotes do monorepo:
   - Alterações em `extension/` $\rightarrow$ typecheck de `@codecanvas/*`.
   - Alterações em `studio/` $\rightarrow$ typecheck de `@codecanvas-studio/assets`.
   - Alterações em `tools/` $\rightarrow$ typecheck de `@code-canvas/tools`.
   - Alterações em `docs/` $\rightarrow$ typecheck de `@code-canvas/docs`.
   - Alterações em `studio/` $\rightarrow$ typecheck de `@codecanvas/*`.
4. Se arquivos de configuração global ou de raiz (como `package.json`, `eslint.config.ts`, `pnpm-workspace.yaml`) forem alterados, o script executa um typecheck completo (excluindo `studio` temporariamente).
5. Executa as validações filtradas em paralelo utilizando o Turborepo (ex: `npx turbo run typecheck --filter=@codecanvas/*`).
...
As regras de filtros estão declaradas no arquivo [package.json](file:///D:/projects/code-canvas/package.json) raiz:
...
O workflow de validação está configurado em [.github/workflows/ci.yaml](file:///D:/projects/code-canvas/.github/workflows/ci.yaml):

- Instala as dependências utilizando o `pnpm install --frozen-lockfile`.
- Valida a tipagem completa do monorepo: `pnpm typecheck --filter=!@codecanvas/*`.
- Valida o linter e o estilo de formatação: `pnpm lint --filter=!@codecanvas/*`.
