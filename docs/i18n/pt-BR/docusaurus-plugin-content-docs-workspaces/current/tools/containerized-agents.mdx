...
Os containers de agentes são executados dentro da mesma rede bridge `codecanvas-mcp-net` que o gateway MCP, comunicando-se com ele por meio de roteamento de DNS interno. Nenhuma exposição de porta no nível do host é necessária para a comunicação entre serviços.
...
graph TD
  ...
  subgraph "Docker: codecanvas-agents"
    Copilot["agent-copilot"]
    Antigravity["agent-antigravity"]
  end

  subgraph "Docker: codecanvas-mcp"
    Gateway["codecanvas-mcp-gateway (elo.internal.tools.mcp:3005)"]
    Github["codecanvas-mcp-github"]
    Context7["codecanvas-mcp-context7"]
    Browser["codecanvas-mcp-browser"]
    Dockerhub["codecanvas-mcp-dockerhub"]
  end

  Dev -- "docker exec -it" --> Copilot
  Dev -- "docker exec -it" --> Antigravity
  Copilot -- "codecanvas-agents-net" --> Host["Host Gateway"]
  Antigravity -- "codecanvas-agents-net" --> Host
  Host -- "codecanvas-mcp-net" --> Gateway
  Gateway --> Github
  Gateway --> Context7
  Gateway --> Browser
  Gateway --> Dockerhub
