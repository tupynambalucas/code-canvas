...
A segurança e a previsibilidade do ambiente são mantidas por meio de redes bridge do Docker isoladas (`codecanvas-agents-net` para agentes e `codecanvas-mcp-net` para ferramentas MCP), evitando o acesso não autorizado entre redes.

#### Configuração de Gateway e Roteamento

- **Gateway Proxy HTTP Fastify:** Um servidor Fastify v5 leve (`codecanvas-mcp-gateway`) executado em Node.js serve como ponto de entrada. Ele expõe a porta `3005` para a máquina host. Usando o `@fastify/http-proxy`, ele encaminha prefixos de rotas (ex: `/github`) para os respectivos containers de backend.
