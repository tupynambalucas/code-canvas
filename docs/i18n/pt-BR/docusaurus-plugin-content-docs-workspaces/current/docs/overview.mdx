---
title: Workspace Docs (CodeCanvas Docs)
sidebar_label: Visão Geral
sidebar_position: 1
slug: /docs
---

O workspace Docs gerencia o hub oficial de documentação para o projeto CodeCanvas, conhecido como **CodeCanvas Docs**. Construído sobre o Docusaurus, ele serve como o studio central do desenvolvedor, abrigando diretrizes arquiteturais, guias de estilo, documentos de planejamento de produto, além de changelogs e roadmaps automatizados do projeto.
...
## @codecanvas-docs/core e Plugins Customizados

O CodeCanvas Docs está configurado com uma estrutura de documentação usando plugins do Docusaurus para segmentar as diferentes áreas do studio do desenvolvedor com barras laterais esquerdas independentes:
...
O CodeCanvas Docs suporta totalmente as localidades Inglês (`en`) e Português (`pt-BR`). Os arquivos localizados espelham a estrutura dos documentos em inglês:
...
Para manter padrões de repositório limpos, o CodeCanvas Docs se integra a scripts de compilação em tempo de build para gravar documentações unificadas em markdown no nível raiz do repositório no momento do deploy:
...
- **Integração de CI/CD**: Ambos os scripts de compilação são executados dentro do workflow `.github/workflows/deploy-docs.yaml` para garantir que os arquivos no nível raiz permaneçam 100% em sincronia com o studio do desenvolvedor.

## Scripts do Workspace

Execute esses comandos a partir do diretório raiz para gerenciar o contexto do CodeCanvas Docs:

| Comando               | Ação                                                                                                  |
| :-------------------- | :---------------------------------------------------------------------------------------------------- |
| `pnpm docs:dev`       | Inicializa o servidor de desenvolvimento local do Docusaurus em `http://localhost:3002`.              |
| `pnpm docs:build`     | Compiles the production build of the Docusaurus studio (runs link check validations).                 |
